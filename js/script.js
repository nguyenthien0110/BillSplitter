...
function calculate() {
  ...
  var positiveDebts = debts.filter(debt => debt.amount > 0).sort((a, b) => b.amount - a.amount);
  var negativeDebts = debts.filter(debt => debt.amount < 0).sort((a, b) => a.amount - b.amount);
  ...
  for (var i = 0; i < positiveDebts.length; i++) {
      for (var j = 0; j < negativeDebts.length; j++) {
          ...
          transactions.push({ from: negativeDebts[j].name, to: positiveDebts[i].name, amount: payment });
  ...
  transactions.forEach(function(transaction) {
      result += transaction.from + " should pay " + transaction.amount.toFixed(2) + " to " + transaction.to + "\n";
  });
